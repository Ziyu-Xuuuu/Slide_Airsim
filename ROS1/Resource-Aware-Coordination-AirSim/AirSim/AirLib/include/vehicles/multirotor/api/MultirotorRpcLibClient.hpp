...
namespace airlib
{
    ...
    {
    public:
        MultirotorRpcLibClient(const string& ip_address = "localhost", uint16_t port = RpcLibPort, float timeout_sec = 36000);

        MultirotorRpcLibClient* takeoffAsync(float timeout_sec = 20, const std::string& vehicle_name = "");
        MultirotorRpcLibClient* landAsync(float timeout_sec = 36000, const std::string& vehicle_name = "");
