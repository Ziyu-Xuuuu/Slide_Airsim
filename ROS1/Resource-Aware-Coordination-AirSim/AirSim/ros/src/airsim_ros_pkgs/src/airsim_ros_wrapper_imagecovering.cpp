/* SENSORS INCLUDED:
IMU, barometer, magnetometer, GPS, RGB camera */
#include <airsim_ros_wrapper_imagecovering.h>
        ...
        for (auto& curr_camera_elem : vehicle_setting->cameras) // iterate over each camera
        {
            ...
            for (const auto& curr_capture_elem : camera_setting.capture_settings)
            {
                ...
                if ( !(std::isnan(capture_setting.fov_degrees)) )
                {
                    ...
                    // image_pose_pub_vec_.push_back(nh_private_.advertise<geometry_msgs::PoseStamped>(
                    //         curr_vehicle_name + "/" + curr_camera_name + "/Scene/camera_pose", 10));
                    ...
                    // cam_info_pub_vec_.push_back(nh_private_.advertise<sensor_msgs::CameraInfo> (curr_vehicle_name + "/" + curr_camera_name + "/" 
                    // + image_type_int_to_string_map_.at(capture_setting.image_type) + "/camera_info", 10));
                    cam_info_pub_vec_.push_back(nh_private_.advertise<sensor_msgs::CameraInfo> (curr_vehicle_name + "/" + curr_camera_name + "/Scene/camera_info", 10));
