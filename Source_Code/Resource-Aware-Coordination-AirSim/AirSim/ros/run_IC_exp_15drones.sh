...
for algorithm in "${algorithms[@]}"; do
    ...
    for random_seed in $(seq 1 50); do
        ...
        log_directory="${log_directory_base}/log_${random_seed}_${algorithm}_0p2mbps"
        ...
        if [ "$algorithm" = "RAG_NN" ]; then
            ...
            roslaunch image_covering_coordination connectivity_mesh_neighbors_DFS.launch num_drones:=15 &
        ...
        # Execute the algorithm with the current random seed for image_covering
        if [ "$algorithm" = "RAG_NN" ]; then
            roslaunch image_covering_coordination image_covering_15drones_RAG.launch algorithm_to_run:=$algorithm random_seed:=$random_seed is_server_experiment:=1 &
        elif [ "$algorithm" = "SG_DFS" ]; then
            roslaunch image_covering_coordination image_covering_15drones_SG.launch algorithm_to_run:=$algorithm random_seed:=$random_seed is_server_experiment:=1 &
        elif [ "$algorithm" = "SG_LP" ]; then
            roslaunch image_covering_coordination image_covering_15drones_SG.launch algorithm_to_run:=$algorithm random_seed:=$random_seed is_server_experiment:=1 &
        else
