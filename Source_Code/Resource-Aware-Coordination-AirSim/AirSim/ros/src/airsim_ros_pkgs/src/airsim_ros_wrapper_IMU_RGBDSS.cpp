#include <airsim_ros_wrapper_IMU_RGBDSS.h>
...
AirsimROSWrapper::AirsimROSWrapper(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private, const std::string& host_ip) : 
    ...
    img_async_spinner_(4, &img_timer_cb_queue_), // a thread for image callbacks to be 'spun' by img_async_spinner_ 
...
void AirsimROSWrapper::initialize_ros()
{
    ...
    // get the drone name and assign to private class variable here
    std::string drone_name;
    nh_private_.getParam("drone_name", drone_name);
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    {   
        ...
        /* if (!(curr_vehicle_name == "Drone1")) // added by SSG to check if latency improves
        {
            continue;
        } */

        if (!(curr_vehicle_name == drone_name_)) // added by SSG to check if latency improves
        {
            continue;
        }
        ...
        for (auto& curr_camera_elem : vehicle_setting->cameras) // iterate over each camera
        {
            ...
            for (const auto& curr_capture_elem : camera_setting.capture_settings)
            {
                ...
                {
                    // ImageType curr_image_type = msr::airlib::Utils::toEnum<ImageType>(capture_setting.image_type);  // msr::airlib::ImageCaptureBase::ImageType::Scene
                    ...
                    // push back RGB, depth, semantic segmentation images 
                    // current_image_request_vec.push_back(ImageRequest(curr_camera_name, msr::airlib::ImageCaptureBase::ImageType::Scene, false, false)); // Scene (RGB)
                    // current_image_request_vec.push_back(ImageRequest(curr_camera_name, msr::airlib::ImageCaptureBase::ImageType::DepthPlanar, true)); // DepthPlanar (depth)
                    // current_image_request_vec.push_back(ImageRequest(curr_camera_name, msr::airlib::ImageCaptureBase::ImageType::Segmentation, false, false)); // Segmentation (semantic segmentation frame)

                    current_image_request_vec = {ImageRequest(curr_camera_name, msr::airlib::ImageCaptureBase::ImageType::Scene, false, false),
                                                ImageRequest(curr_camera_name, msr::airlib::ImageCaptureBase::ImageType::DepthPlanar, true),
                                                ImageRequest(curr_camera_name, msr::airlib::ImageCaptureBase::ImageType::Segmentation, false, false)} ;
                    ...
                    // image_pub_vec_.push_back(image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/" + image_type_int_to_string_map_.at(0), 1));
                    // image_pub_vec_.push_back(image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/" + image_type_int_to_string_map_.at(1), 1));
                    // image_pub_vec_.push_back(image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/" + image_type_int_to_string_map_.at(5), 1));
                    // image_pub_vec_ = {image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/" + image_type_int_to_string_map_.at(0), 1), 
                    //                 image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/" + image_type_int_to_string_map_.at(1), 1),
                    //                 image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/" + image_type_int_to_string_map_.at(5), 1)};

                    // image_pub_vec_ = {image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/Scene", 1), 
                    //     image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/DepthPlanar", 1),
                    //     image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/Segmentation", 1)};
                        image_pub_vec_ = {image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/Segmentation", 1), 
                        image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/DepthPlanar", 1),
                        image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/Scene", 1)};
            ...
            {             
                ...
                switch (sensor_setting->sensor_type)
                {
                    ...
                    default:
                    {
                        ...
                        std::cout << "Warning: Unexpected sensor type encountered OR disabling sensor type " << sensor_name << std::endl;
                    }
                }
        ...
        vehicle_ros->sensor_pubs.resize(sensors.size());
        std::copy(sensors.begin(), sensors.end(), vehicle_ros->sensor_pubs.begin());
...
/* void AirsimROSWrapper::set_nans_to_zeros_in_pose(const VehicleSetting& vehicle_setting, LidarSetting& lidar_setting) const
{
    if (std::isnan(lidar_setting.position.x()))
        lidar_setting.relative_pose.position.x() = vehicle_setting.position.x();

    if (std::isnan(lidar_setting.position.y()))
        lidar_setting.relative_pose.position.y() = vehicle_setting.position.y();

    if (std::isnan(lidar_setting.position.z()))
        lidar_setting.relative_pose.position.z() = vehicle_setting.position.z();
...
{    
    ...
    try
    {
        ...
        {
            const std::vector<ImageResponse>& img_response = airsim_client_images_.simGetImages(airsim_img_request_vehicle_name_pair.first, airsim_img_request_vehicle_name_pair.second); // gets RGB, depth, SS frames here
            // lck.unlock();
            ...
            if (img_response.size() == airsim_img_request_vehicle_name_pair.first.size()) // synchronizes RGB, depth, SS frames until all received from single call
            {
...
sensor_msgs::ImagePtr AirsimROSWrapper::get_depth_img_msg_from_response(const ImageResponse& img_response, const std::string frame_id)
{
...
{    
    ...
    for (const auto& curr_img_response : img_response_vec)
    {
        ...
        if (img_response_idx_internal == 0)
        {   
            publish_camera_tf(curr_img_response, vehicle_name, curr_img_response.camera_name);
        ...
        {
            image_pub_vec_[img_response_idx_internal].publish(get_depth_img_msg_from_response(curr_img_response, curr_img_response.camera_name + "_optical"));
        }
        // Scene / Segmentation / SurfaceNormals / Infrared
        else
        {
            image_pub_vec_[img_response_idx_internal].publish(get_img_msg_from_response(curr_img_response, curr_img_response.camera_name + "_optical"));
        }
